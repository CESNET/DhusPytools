...
def parse_kafka(output: str, namespace: str):
    metrics = []
    for line in output.splitlines():
        ...
        labels = f'namespace="{namespace}",group="{group}",topic="{topic}",partition="{partition}"'
...
if __name__ == "__main__":
    ...
    COMMANDS = [
        ("relay", ["/root/bin/kafka-get-queue-informations.sh", "relay", "-q"]),
        ("default", ["/root/bin/kafka-get-queue-informations.sh", "default", "-q"]),
    ]
    ...
    for namespace, command in COMMANDS:
        proc = subprocess.run(command, capture_output=True, text=True, check=True)
        metrics += parse_kafka(proc.stdout, namespace)
